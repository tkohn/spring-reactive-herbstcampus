...
public class FluxAndMonoSubscribeTest {
  ...
  public void coldSubscriberTest() {
    Flux<String> flux = Flux.just("Go", "Java", "C", "JavaScript")
      .doOnNext(System.out::println)
  ...
  public void hotSubscriberTest() {
    ...
    hot.onNext("Java");
    hot.onNext("C++");
    Flux<String> flux = hot.publish().autoConnect().map(String::toUpperCase);
    hot.onNext("Go");

    flux.subscribe(name -> System.out.println("subscribe 1: " + name));
    hot.onNext("Scala");
    hot.onNext("TypeScript");

    flux.subscribe(name -> System.out.println("subscribe 2: " + name));
    hot.onNext("Closure");
    hot.onNext("php");
  }
